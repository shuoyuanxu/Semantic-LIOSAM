...
class Network(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        if (config.name == 'Cunjia_liu'):
            self.class_weights = DP.get_class_weights('Cunjia_liu')
        ...
        for j in range(self.config.num_layers):
            if j < config.num_layers - 1:
                d_in = d_out + 2 * self.config.d_out[
                    -j - 2]
    ...
    def forward(self, end_points):
        ...
        f_encoder_list = []
        for i in range(self.config.num_layers):
            f_encoder_i = self.dilated_res_blocks[i](features, end_points['xyz'][i],
    ...
    def nearest_interpolation(feature, interp_idx):
        ...
        interpolated_features = torch.gather(feature, 2,
...
class Building_block(nn.Module):
    ...
    def forward(self, xyz, feature, neigh_idx):
        """
        xyz: point clouds
        feature: corresponding features
        neigh_idx: the idx of neighbouring points
        """
    ...
    def relative_pos_encoding(self, xyz, neigh_idx):
        """xyz: point clouds
        "neighbor_idx；the idx of neighbouring points"""
